...
- 完成孩子端心愿兑换申请奖励反馈：仅在兑换申请接口成功并更新心愿、积分与流水状态后展示约 2.2 秒的轻量庆祝弹层。
- 新增独立客户端组件 `WishRedeemCelebration`；根据体验反馈移除白色信息卡、标题、心愿名称、积分和状态说明，仅保留礼盒、柔光与星光特效。
- 兑换后不再展示可见成功气泡，业务状态由心愿卡“待家长确认”标签表达；保留仅供读屏使用的 `role="status"` 成功播报。
- 使用 Image Gen 生成礼盒、星星与彩屑插画，经本地色键抠图输出透明 WebP 到 `src/frontend/public/effects/wish-redeem-celebration.webp`。
- 奖励反馈支持点击背景、`Escape` 和自动计时关闭；支持 `prefers-reduced-motion`，减少动态效果时隐藏漂浮星光并仅保留淡入。
- 使用 Web Audio API 合成约 0.8 秒三音提示；孩子端提供默认开启的音效开关，并使用 `localStorage` 持久保存静音选择。
- 优化孩子端音效开关的胶囊布局：补齐 44px 触控高度、稳定内边距和不可压缩图标，并区分开启、静音、悬停、按下与键盘焦点状态，修复窄宽下文字贴边和控件变形。
- 动效、音频与本地存储异常均安全降级，不修改后端接口、数据库、积分事务或心愿状态机。
- Playwright 心愿主流程增加弹层内容、单次触发、键盘关闭、静音持久化、减少动态效果和 390px 视口覆盖。

## 2026-06-11

- 修复孩子端心愿列表中“积分不足”待兑换卡片的进度条被右侧操作按钮挤短的问题；桌面端进度区改为横跨整张卡片，保持不同状态下的进度条长度一致。
- 本次仅调整前端布局，不涉及接口或产品行为变更，`docs/api.md` 和 `docs/prd.md` 无需更新。
- 任务主状态收敛为四态，增加兼容迁移并移除运行时 AI 分支。
- 任务默认积分改为 1，新增备注，删除规则收紧为无提交的 `pending`。
- 提交和审核链路改为事务写入；审核使用任务、提交行锁和唯一审核约束，积分发放幂等。
- 新增兑换请求审计表，拒绝兑换必须填写原因并按请求 ID 退款。
- 新增家庭唯一孩子上下文、全部提交时间线、上海时区逾期判断和受保护图片读取。
- 家长看板增加孩子积分与 14 天指标；孩子心愿增加积分进度、退款金额和原因。
- 打卡页增加本地预览、单图失败提示和重试，支持 0 张或最多 9 张的任务类型。
- 全新 schema 不再创建 AI、错题和周报表；已有数据库通过迁移保留旧扩展表。
- 后端单测增至 35 个；增加 API 集成、Playwright E2E、Node 22 + MySQL GitHub Actions。
- 数据库升级脚本：`src/backend/db/migrations/20260611_prd_alignment.sql`。

## 2026-06-08

- 设计并接入“学习伙伴抱书与星星”的卡通 SVG 标志，全站顶部品牌区与登录页已应用新标志。
- Next.js metadata 已配置 SVG favicon 与 shortcut icon，保持页面品牌与浏览器图标一致。
- 调整心愿兑换积分时点：孩子申请兑换时在事务内锁定愿望和积分账户、立即扣分并写入 `wish_redeem` 流水，避免多个待确认申请重复占用同一余额。
- 新增家长拒绝兑换接口和前端操作；拒绝后写入 `wish_refund` 流水、返还积分、减少净累计消费，并将心愿恢复为可兑换。
- 家长确认兑换不再检查余额或重复扣分，只完成心愿状态；旧 `redeem_requested` 数据迁移时统一退回可兑换。
- 每次兑换申请使用独立请求 ID，积分流水唯一键调整为来源与原因组合，支持同一心愿被拒绝后再次申请。
- 兼容升级前没有兑换请求 ID、也未预扣积分的待确认记录：家长拒绝时直接恢复为可兑换，不生成虚假退款流水。
- 修正确认兑换接口响应结构，统一返回 `{ wish }`，避免后端已确认成功但家长端因读取不到 `result.wish` 发生运行时错误。
- 更新积分心愿 Service 测试、Playwright 主流程和退款重试流程，并同步 API、PRD、技术文档与 OpenAPI。
- 修复家长任务管理页状态筛选重复显示两个“待家长确认”的问题，移除当前流程不再使用的 `ai_checking` 筛选项，保留有效的 `parent_review` 筛选。
- 调整家长端与孩子端日历页：将“上月 / 今天 / 下月”从页面说明卡移动到月历控件标题栏上方，桌面端和移动端使用一致的完整按钮文案。
- 重构孩子端任务列表：将“今日 / 逾期 / 已完成”状态筛选、筛选说明、任务数量和任务清单合并为同一个任务面板，并参考家长端统一列表卡片的边框、间距和信息层级。
- 完成第一轮 P0 内部演示优化：手机端使用 64px 顶部栏和固定底部“任务 / 日历 / 心愿”导航，身份与退出收进顶部紧凑区域。
- 日历在手机端仅展示日期、状态点和任务数，完整任务标题统一放在选中日期面板；桌面端继续展示最多三条摘要。
- `GET /tasks/:taskId` 增加 `latestReview`，孩子结果页展示家长补充原因并提供“补充打卡”直达入口；未提交任务不再显示结果入口。
- AI 检查选项改为禁用并标记“暂未开放”；新提交直接进入家长确认，不再产生新的 `ai_checking` 状态。
- 家长看板“今日任务”改为“当前任务”，待确认统计覆盖 `submitted`、`ai_checking`、`parent_review`，并按待确认、需补充、今日/逾期待完成、未来任务排序。
- 增加角色路由守卫：孩子访问家长页面跳转 `/child`，家长访问孩子页面跳转 `/parent`。
- 新增前后端业务日期工具，统一使用 `Asia/Shanghai`；后端支持 `APP_TIMEZONE`，并覆盖上海凌晨、月末和跨年单测。
- 移除登录页测试账号卡、导航 seed 中的 Demo 昵称、固定孩子 Demo 名称，以及首页 URL 上传、Qiniu、联调等过期文案。
- 更新 Playwright 任务流与日历测试，新增补充打卡闭环、错误角色跳转和 390px 无横向滚动验证。
- 孩子发起心愿兑换后，成功提示改为显示在对应心愿卡片操作区的气泡反馈，并通过 `role="status"` 播报结果，避免页面级提示不够醒目。
- 修复后端 `pnpm test` glob 漏跑 feature 单测的问题；后端 24 个单测通过，前后端类型检查及后端构建通过。
...
- 新增愿望接口：孩子提交愿望，家长设置所需积分并通过或驳回，孩子申请兑换，家长处理兑换结果。
- 愿望兑换积分事务后续扩展为申请时预扣、拒绝时退款，并按兑换请求记录唯一流水。
